...
declare module 'express-session' {
  interface SessionData {
    userId?: number;  // Adicionando userId à sessão do Express
  }
}
...
declare global {
  namespace Express {
    interface Request {
      user?: { username: string };  // Definindo a propriedade `user` no Request
    }
  }
}
...
// Adiciona o tipo `user` no `Request`
...
type WithUserHandler<T> = (req: WithUser<T>, res: Response, next: NextFunction);

// Declara o tipo da sessão do express para incluir o `userId`
declare module 'express-session' {
  interface SessionData {
    userId?: number;
  }
...
export function maybeWithUser(handler: MaybeWithUserHandler<Request>): RequestHandler {
  return function (req, res, next) {
    if (req.session.userId) {
      ...
      if (user) {
        req.user = { username: user.username };  // Agora, TypeScript sabe que req tem a propriedade `user`
